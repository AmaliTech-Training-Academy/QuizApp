...
const AccountSettings = () => {
  ...
  return (
    ...
    {showModal && <DeleteModale showModal={showModal} setShowModal={setShowModal}/>}
    <section>
    ...
    <div className=' mt-[127px] mb-[316px] lg:justify-center xl:w-[70%]  m-auto'>
      ...
      <div className='flex gap-[1.5rem] mb-[104px]'>
        <div className='rounded-[50%] w-16 h-16 bg-white-700 flex justify-center shadow-lg shadow-[rgba(0, 0, 0, 0.25)]'>
          <img className='border-2 rounded-[50%] h-14 w-14 bg-[#b3b3b3] self-center' src={person} alt="person image"/>
        </div>
    ...
    {/* selections */}
      ...
      <div className='font-semibold text-base tracking-wid w-max'>
        <div className='flex gap-[0.5rem] p-[0.5rem] content-center '>
          <MdOutlinePeople className='text-[#0267FF] self-center w-[1.5rem] h-[1.5rem]'/>
          <p>General</p>
        </div>
        <div className='flex gap-[0.5rem] p-[0.5rem] content-center mt-[50px]'>
        ...
        <div className='flex gap-[0.5rem] p-[0.5rem] content-center text-[#FF0000] mt-[50px]' onClick={()=>setShowModal(!showModal)}>
    ...
    <div className='lg:ml-[5rem]'>
        <div className='flex  w-fit'>
          <div className='rounded-[50%] w-40 h-40 bg-white-700 flex justify-center shadow-lg shadow-[rgba(0, 0, 0, 0.25)] mr-[2rem] '>
            <img className='border-2 rounded-[50%] h-36 w-36 bg-[#b3b3b3] self-center' src={person} alt="person image"/>
          </div>
          <button className='p-[0.5rem] w-max h-fit self-center mr-[0.55rem]'>Upload New</button>
          <button className='p-[0.5rem] w-max h-fit self-center bg-white text-[#1D2939] border-none'>Delete Avatar</button>
        </div>

        <div className='mt-[41px] grid grid-cols-2 gap-[1.5rem] w-fit'>
          <div>
            <label>Full Name* </label>
            <div className='mt-[8px] w-fit'><input className='border py-[0.5rem] px-[1rem] rounded'/></div>
          </div>
          <div>
            <label>Email*</label>
            <div className='mt-[8px] w-fit'><input className='border py-[0.5rem] px-[1rem] rounded'/></div>
          ...
          <div className='flex gap-[3rem]'>
            <div><input className='border' type='radio'/> <label >Male</label></div>
            <div><input className='border' type='radio'/> <label >Female</label></div>
          </div>
        </div>
        <button className='p-[0.5rem] w-max h-fit self-center mt-[34px]'>Save changes</button>
      </div>
  )
};
  ...
  return(
    <div className='mt-[100px]'>
      <div className='mb-[48px]'>
        ...
        <div className='mt-[8px] w-fit'><input  className='border py-[0.5rem] px-[1rem]'/></div>
      </div>
...
export const DeleteModale = ({showModal, setShowModal})=>{
  return(
    <div className='fixed  inset-x-0 inset-y-0 bg-gray-600 flex content-center justify-center'>
    <div className='p-[1.5rem] w-[22.75rem] bg-white border border-black rounded-lg m-auto ' >
      <p className='mb-[8px] text-[#1D2939] font-semibold'>Delete Account</p>
      <p className='mb-[16px]'>Hey, if you're absolutely sure you want to delete your account, we got you covered. This Process cannot be undone</p>
      <div className='flex justify-between'>
        <button className='border-[#B3B3B3] px-5 py-2 bg-[white] text-black' onClick={()=>setShowModal(false)}>No keep it</button>
        <button className='bg-[#FF0000] border-none px-5 py-2'>Yes, Delete Account</button>
      </div>
    </div>
    </div>
  )
